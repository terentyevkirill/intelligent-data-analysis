...
# автоматический ввод точек (25 точек, распределенных поровну по матрице)
class1 = np.array(
    [[0.05, 0.91],
     [0.14, 0.96],
     [0.16, 0.9],
...
a_arr = [[round(x, 2),round(y, 2)] for x in np.arange(0, 0.61, 0.01) for y in np.arange(0, 1.02, 0.02)]
...
p5, = plt.plot(class5[:, 0], class5[:, 1], 'oy')


p6, = plt.plot(class6[:, 0], class6[:, 1], '^g')

plt.legend([p1, p2, p5, p6], ["class1", "class2", "class5", "class6"])
plt.xlabel("x")
plt.ylabel("y")
plt.show()
